...
docker ビルダーは docker image 形式の tar ファイルを生成でき、システムに docker CLI がなくても動作し、マルチアーキテクチャをサポートし、ビルド対象は linux プラットフォームのみです。
...
複数のターゲットをビルドする必要がある場合は、`docker image targets` を複数回記述します。
...
`pip.download` はプロジェクトの依存関係を宣言するために使用します。複数の依存関係がある場合は、`pip.download` を複数記述してください。
