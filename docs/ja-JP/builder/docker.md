...
docker ビルダーは docker image 形式の tar ファイルを構築でき、システムに docker CLI がなくても動作し、マルチアーキテクチャをサポートし、ビルド対象は linux プラットフォームのみです。
...
`pip.download` はプロジェクトの依存関係を宣言するために使用し、複数の依存がある場合は `pip.download` を複数記述します。
